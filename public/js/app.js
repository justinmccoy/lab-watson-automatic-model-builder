...
var	ResultsCtrl = ['$scope',	'$modalInstance',	'rspHeader', 'rspData', function ResultsCtrl($scope,	$modalInstance, rspHeader, rspData) {
	...
	if (rspData.values[0][12] == 1)  // PREDICTION
	   formattedData.push('Yes');
    else
	   formattedData.push('No');

   // Format confidence
    if (rspData.values[0][12] == 1)  // CONFIDENCE
       confidence = (rspData.values[0][11][1] * 100).toFixed(2) + '%';
    else
	   confidence = (rspData.values[0][11][0] * 100).toFixed(2) + '%';
